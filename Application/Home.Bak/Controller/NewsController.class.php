...
class NewsController extends EmptyController{
	...
	public function index(){	
		...
		$newslist=M('news')->field('id,cover,title,author,click,keywords,addtime,description')->where(array('isshow'=>1))->order('sortorder, addtime DESC')->limit($limit)->select();
		$ids=array();
		...
		if (!empty($ids))
		$this->hot=M('news')->field('id,title')->where(array('isshow'=>1,'id'=>array('not in', $ids)))->order('click DESC')->limit(8)->select();
		else $this->hot=false;	
		//----- 页面信息获取---//
		$pageinfo=array(
				'pagetitle'=>'News List',
	...
	public function detail(){
		$nid=I('nid', 0,'intval');
		if ($news=M('news')->field('coverrealpath,isshow,sortorder', true)->where(array('isshow'=>1, 'id'=>$nid))->find()){
			M('news')->where(array('id'=>$nid))->setInc('click',1);
			$news['content']= htmlspecialchars_decode($news['content']);
			//上一条新闻
			$prevnews=M('news')->field('id,title')->where(array('isshow'=>1,'id'=>array('LT',$nid)))->order('id DESC')->find();
			$nextnews=M('news')->field('id,title')->where(array('isshow'=>1,'id'=>array('GT',$nid)))->order('id ASC')->find();
			//最新新闻			
			$this->newest=M('news')->field('id,title,cover,click,addtime')->where(array('id'=>array('not in',array($nid)),'isshow'=>1))->order('addtime DESC')->limit(4)->select();
			$this->news=$news;
			$this->prev=$prevnews;
			$this->next=$nextnews;
			//----- 页面信息获取---//
