...
class SearchController extends EmptyController{
	...
	public function sq (){
		...
		if(!empty($q)){
			...
			//-------------------引入分页类;
			$results=M('goods')->field('id,goodsname,price,goodslink, score, favnum, click, description,goodsimg')->where($where)->order('sortorder, addtime DESC')->limit($limit)->select();
		}else{
			$results=false;
		}
