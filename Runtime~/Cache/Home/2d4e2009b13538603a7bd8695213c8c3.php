...
<!-- 公共才css js 部分 --></head>
<body>
<link rel="stylesheet" type="text/css" href="/assets/css/default/goods/category.css" />
...
<!-- 商品分类 页面 -->
 	<div class="category">
			<div class="container">		
				<?php if($relatecates): ?><div class="cats_display">
						<ul>
							<?php if(is_array($relatecates)): foreach($relatecates as $key=>$cate): ?><li class="cate_one_item"><a href="<?php echo U('Goods/index',array('cid'=>$cate['id'],'justfrom'=>'catelist','do'=>'fsub'));?>" class="catelink <?php if(($cate["id"]) == $curcateid): ?>active<?php endif; ?>"><?php echo ($cate["catename"]); ?><span style="margin-left: 3px;color: <?php if(($cate["id"]) == $curcateid): ?>#FFF<?php else: ?>》#00A7E1<?php endif; ?>;"></span></a></li><?php endforeach; endif; ?>		 					
						</ul>
				...
				<!-- 产品列表 -->
					...
					<div class="sort-bar" id="main-nav">				       
				        ...
				        <div class="nch-sortbar-array"> Sort By：
				          <ul>
				            <li <?php if(empty($s)): ?>class="selected"<?php endif; ?>><a href="<?php echo U('Goods/index',array('cid'=>$curcateid,'justfrom'=>'catelist'));?>" title="default">default</a></li>
				            <li <?php if(($s) == "2"): ?>class="selected"<?php endif; ?>><a href="<?php echo U('Goods/index',array('cid'=>$curcateid,'s'=>2,'click'=>$sale,'justfrom'=>'catelist'));?>" <?php if(($sale) == "2"): ?>title="click from low to high sort" class="asc"<?php else: ?>title="click from high to low sort"<?php endif; ?>>click<i></i></a></li>
				            <li <?php if(($s) == "3"): ?>class="selected"<?php endif; ?>><a href="<?php echo U('Goods/index',array('cid'=>$curcateid,'s'=>3,'pop'=>$pop,'justfrom'=>'catelist'));?>" <?php if(($pop) == "2"): ?>title="popular from low to high sort" class="asc"<?php else: ?>title="click from high to low sort"<?php endif; ?>>popular<i></i></a></li>
				            <li <?php if(($s) == "4"): ?>class="selected"<?php endif; ?>><a href="<?php echo U('Goods/index',array('cid'=>$curcateid,'s'=>4,'price'=>$price,'justfrom'=>'catelist'));?>" <?php if(($price) == "2"): ?>title="price from low to high sort" class="asc"<?php else: ?>title="click from high to low sort"<?php endif; ?>>price<i></i></a></li>
				          </ul>
				        </div>				        
      				</div>
      
			<ul class="jumei-xp-list">
				<?php if($goods): if(is_array($goods)): foreach($goods as $idx=>$g): ?><li <?php if((($idx+1)%3)==1): ?>class="noml"<?php elseif((($idx+1)%3)==0): ?>class="nomr"<?php endif; ?>>
						<div class="jumei-img-box" id="<?php echo ($g["id"]); ?>">
								<img class="scrollLoading" data-url="<?php echo ($g["goodsimg"]); ?>" src="<?php echo ($g["goodsimg"]); ?>" alt="<?php echo ($g["goodsname"]); ?>">
								<a href="<?php if(empty($g["goodslink"])): echo U('Goods/detail',array('gid'=>$g['id'], 'justfrom'=>'catelist')); else: echo ($g["goodslink"]); endif; ?>" class="op buy"><i class="fa fa-shopping-cart"></i>buy now</a>
								<a href="javascript:" class="op cart add_wish"><i class="fa fa-shopping-basket"></i>add to cart</a>
						</div>
							<a href="<?php echo U('Goods/detail',array('gid'=>$g['id'], 'justfrom'=>'catelist'));?>" class="glink" title="<?php echo ($g["goodsname"]); ?>"><?php echo (mb_substr($g["goodsname"],0,40)); ?>..</a>
							<div class="intro_box">
								<div class="xp-price red-color">
									<em>$</em><span><?php echo ($g["price"]); ?></span>
								</div>
								<div class="price_icon_wrap fr">
									<i class="fa fa-eye"></i><?php echo ($g["click"]); ?>
								</div>
							</div>
						</li><?php endforeach; endif; ?>
				<?php else: ?>
					<li style="padding: 100px 0;text-align: center;font-size: 2em;width: 100%;color: #898989;">Sorry ! No datas at this category.</li><?php endif; ?>
				</ul>
