...
(function($){
	...
	$.jMenu = {
		...
		init:function(options){
			...
			$("#Menulist a:not(.fNiv)").each(function(){
				var $thisChild = $(this);
			...
			$('#Menulist li a.fNiv').bind({
				mouseover:function(){
					var $this = $(this);
					var $child = $this.next();
					ULWidth = $.jMenu._returnUlWidth($this);
					$.jMenu._closeList($("#Menulist ul"));
					if($child.is(':hidden'))
		...
		_showNextChild:function(el){
			if($.jMenu._IsParent(el))
			{
				...
				if(ChildList.is(":hidden"))
				{
					...
					.css({
						top : position.top,
						left : ($(el).offset().left +ULWidth*2)>$(window).width() ? position.left -ULWidth : position.left +ULWidth,
						width : ULWidth
					})
					.children().css({
						width:ULWidth
