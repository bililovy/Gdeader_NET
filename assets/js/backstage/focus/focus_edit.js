KindEditor
    .ready(function (K) {
        ...
        var title = $('input[name="title"]'), focusname = $('input[name="focusname"]'), linkurl = $('input[name="linkurl"]'), image = $('input[name="image"]'), isshow = $('input[name="isshow"]'), focusid = $('input[name="focusid"]'), sortorder = $('input[name="sortorder"]'), desc = $('textarea[name="desc"]'), btntext = $('input[name="btntext"]'), showtype = $('select[name="showtype"]');
        title.blur(function () {
            title_func($(this));
        });
        ...
        $('input[name="submit"]')
            .click(
                function () {
                    var $this = $(this);
                    var focusidval = parseInt(focusid.val());
                    if (focusidval > 0) {
                                    ...
                                    function (back) {
                                        if (back.status == 1) {
                                            $this
                                                .siblings("i")
                                                .html(
                                                    '<span class="doing">更新成功，正在返回之前页面……</span>');
                                            setTimeout(function () {
                                                window.history.go(-1);
                                            }, 1000);
                                        } else {
                        ...
                        easyDialog.open({
                            container: {
                                header: "消息提醒窗口",
                                content: "当前操作缺少必需参数，请返回之前页面重新进入尝试！"
                            },
                            autoClose: 3000
                        });
                    }
...
function linkurl_func(o) {
    ...
    if (is_empty(val)) {
        ...
        if (!is_url(val)) {
            setTips(o, "此url地址不可用，请输入可访问的URL地址，必须带“http://或https://”");
            return false;
        }
...
function file_change(e) {
    ...
    var $this = $("div#show-cover .img img");
}
