KindEditor
		.ready(function(K) {
			...
			var title = $('input[name="title"]'), keywords = $('input[name="keywords"]'),cover = $('input[name="cover"]'), description = $('textarea[name="description"]'), isshow = $('input[name="isshow"]'), sortorder = $('input[name="sortorder"]'), author = $('input[name="author"]'), newsid = $('input[name="newsid"]'), click = $('input[name="click"]');
			title.blur(function() {
				title_func($(this));
			});
													...
													function(back) {
														if (back.status == 1) {
															easyDialog
																	.open({
																		container : {
																			content : "成功更新此条新闻，即将跳转……"
																		},
																		autoClose : 2000,
																		callback : function() {
																			window.location.href = $newsHomeUrl;
																		}
																	});
														} else {
															easyDialog
			...
			function author_func(o) {
				...
				if (is_empty(val)) {
					...
					var num_reg = /\d/g;
					if (num_reg.test(val) || has_specialchars(val)) {
						setTips(o, "发布者只能为字母或汉字");
						return false;
					} else {
